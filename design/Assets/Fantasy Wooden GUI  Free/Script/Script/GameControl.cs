...
public class GameControl{
    ...
    public GameControl()
    {
		...
		this.gameSence=new GameScene(4);	//创建场景(3代表和劲敌战斗)
        this.CalculateReward();     //计算结算奖励
        this.gameInfo = new List<GameInfo> { }; //初始化战斗信息

        //测试存档
        //SaveManage saveManage = new SaveManage();
        //saveManage.Saves[0] = new Save(1, new Task(1, "收集 5 个 野猪肉 ",
        //        new Dictionary<string, TaskRequireNumber>{
        //            { "野猪肉",new TaskRequireNumber(0,5)}
        //        },
        //        new List<Good> { }, 0), gameSence.allList[0], gameSence.allList[1], gameSence.allList[2], new BackPack(), "地图1", new Vector3(0, 0, 0));
        //saveManage.Saves[0].Index = 0;
        //saveManage.Save(0);
        //saveManage.InfoLoad();
        //saveManage.PrintInfo();
    }

    public GameControl(int type)
    {
        this.orderList = new List<Order> { }; //初始化指令列表
        this.round = 1;     //第一回合
        this.resultType = 0;    //游戏未结束
        this.gameSence = new GameScene(type);	//创建场景(3代表和劲敌战斗)
        this.CalculateReward();     //计算结算奖励
        this.gameInfo = new List<GameInfo> { }; //初始化战斗信息
    }
   ...
   public void PrintAllBuffInfo()
    {
        foreach(Person p in gameSence.allList)
        {
   ...
   public void PrintGameInfo()
    ...
    //刷新buff时间
    public void RefreshBuffTime(Person p)
    {
        for (int i = 0; i < p.buffs.Count; i++)
        {
            p.buffs[i].Time--;
            if (p.buffs[i].Time <= 0)
            {
                p.buffs.Remove(p.buffs[i]);
            }
        }
    }
    ...
    public void ExcuteOrderList(){
        ...
        foreach (var order in this.orderList)
        {
            ...
            {   //1011是防御
                if (((SkillUseStruct)order).SkillID == 1011)
                {
                    ...
                    {
                        RefreshBuffTime(gameSence.allDict[order.CasterID]);
        ...
        foreach (var person in gameSence.speedList){
            ...
            if (order!=null){
                ...
                //执行攻击
                if (!gameSence.allDict[order.CasterID].IsDie() && gameSence.allDict[order.CasterID].AttackIsOk==true)
                {
